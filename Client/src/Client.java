		...
		try
		{
			if(!flag)
			{
				if (myUsername.length() < 3) {
					loginMessage2 = "Please enter a longer username";
					return;
				} else if (myPassword.length() < 5) {
					loginMessage2 = "Please enter a more secure password";
					return;
				}
				loginMessage1 = "";
		...
		try {
			...
			switch(pktType) {
				...
				case 248:
					...
					if (i5 == 3824) {                    	
						RSInterface kk = RSInterface.interfaceCache[3900];	
						for (int zz = 0; zz < kk.inv.length; zz++) {						                  
						if (kk.inv[zz] <= 0) {						                                		
						RSInterface.interfaceCache[3824].children[4+zz] = 67005;						           
						} else {                             
						RSInterface.interfaceCache[3824].children[4+zz] = 67100+(zz*6);	    
						}
