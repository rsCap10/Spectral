...
public final class RSInterface {
	...
	public static void shop(TextDrawingArea[] tda) {
        for (int i = 0; i < 40*6; i+= 6) {
         RSInterface place = addInterface(67100+i);
  addHoverButton(67101+i, "Shop/FRAME", 0, 48, 52, "Info", 2544, 67104+i, 5555);
  addHoveredButton(67104+i, "Shop/HOVER", 0, 50, 52, 67105+i);
         addText(38500+i, "@whi@100K", tda, 0, -1, true, true);
         addSprite(67103+i, 0, "Shop/CURRENCY");
         place.totalChildren(4);
         place.child(0, 67101+i, 0, 0);
   place.child(1, 67104+i, 0, 0);
         place.child(2, 38500+i, 30, 38);
         place.child(3, 67103+i, 3, 39);
        }
        RSInterface shop = addInterface(3824);
        addHoverButton(67001, "Shop/CLOSE", 1, 16, 16, "Close Window", 250, 67002, 3);
  addHoveredButton(67002, "Shop/CLOSE", 2, 16, 16, 67003);
        addSprite(67000, 0, "Shop/SHOP");
        addSprite(67005, 1, "Shop/SHOP");
        RSInterface titleText = interfaceCache[3901];
        titleText.centerText = true;
        RSInterface items = interfaceCache[3900];
        items.width = 8;
        items.height = 5;
        items.invSpritePadX = 28;
        items.invSpritePadY = 21;
        shop.totalChildren(45);
        shop.child(0, 67000, 0-5, 0);
        shop.child(1, 3901, 230-5, 12);
        shop.child(2, 67001, 484-5, 11);
        shop.child(3, 67002, 484-5, 11);
        for (int i = 0; i < 40*6; i+= 6) {
         shop.child(4+(i/6), 67100+i, getShopFrameX(i/6)-8-5, getShopFrameY(i/6)-5);
        }
        shop.child(44, 3900, 34-5, 37);
       }
        
    public static int getShopFrameX(int k) {
    	int x = 36;
    	for (int i = 0; i < 5; i++) {
    		if (k >= 8) {
    			k -= 8;
    ...
    public static int getShopFrameY(int k) {
    	int y = 37;
    	if (k >= 0 && k <= 7) { y = 37; }
    	if (k >= 8 && k <= 15) { y = 90; }
    	if (k >= 16 && k <= 23) { y = 143; }
